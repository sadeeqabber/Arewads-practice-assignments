# Arewads-practice-assignments

**Personal practice & assignment repository**  
Arewa Data Science Academy · Cohort 4.0 (2026)

This repository contains my completed lab notebooks and practice work for the Arewa Data Science Bootcamp.

---

## About

- **Student**: Muhammad Abubakar Sadiq  
- **GitHub**: [sadeeqabber](https://github.com/sadeeqabber)  
- **Cohort**: Arewa Data Science Bootcamp – Cohort 4.0 (2026)  
- **Course**: Introduction to Python / Python for Data Science  
- **Background**: Medical Laboratory Scientist transitioning into Bioinformatics

---

## Repository Structure
Arewads-practice-assignments/
├── assignments/               # Completed lab notebooks
│   ├── week1_lab_completed.ipynb
│   └── week2_lab_completed.ipynb
├── notebooks/
├── scripts/
├── projects/
├── README.md
└── .gitignore

---

## Progress Tracker

| Week | Topic                          | Status     | Notes                          |
|------|--------------------------------|------------|--------------------------------|
| 1    | First Programs & the Shell     | ✅ Completed | Submitted                      |
| 2    | Data Types & Expressions       | ✅ Completed | Submitted                      |
| 3    | Making Decisions               |   inprogesss |                                |
| 4    | Repetition with Loops          |            |                                |
| 5    | Loop Patterns & Nested Logic   |            |                                |
| 6    | Strings & Lists                |            |                                |
| 7    | Dictionaries & First Functions |            |                                |
| 8    | Functions in Depth & Capstone  |            |                                |

---

## Submitted Assignments

- [Week 1 Lab](assignments/week1_lab_muhammad_abubakar_sadiq.ipynb)
- [Week 2 Lab](assignments/week2_lab_muhammad_abubakar_sadiq.ipynb)
- [Week 3 Lab](assignments/week3_lab_muhammad_abubakar_sadiq.ipynb)

---

## Official Course Repository

I also work with the official materials from:  
**[arewadataScience/ArewaDS-Introduction-to-Python-2026](https://github.com/arewadataScience/ArewaDS-Introduction-to-Python-2026)**

---

**Happy coding!**  
*Arewa Data Science Academy – Cohort 4.0*
